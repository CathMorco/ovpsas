<nav x-data="{ open: false }" class="bg-[#800000] border-b border-red-900 sticky top-0 z-50 shadow-md">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-red-900 text-white">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="url('/')" :active="request()->is('/')" class="text-white hover:bg-red-800 block px-3 py-2 text-base font-medium">
                {{ __('Home') }}
            </x-responsive-nav-link>
            
            @auth
                <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-white hover:bg-red-800 block px-3 py-2 text-base font-medium">
                    {{ __('Dashboard') }}
                </x-responsive-nav-link>
            @endauth

            <x-responsive-nav-link :href="url('/about')" :active="request()->is('about')" class="text-white hover:bg-red-800 block px-3 py-2 text-base font-medium">
                {{ __('About Us') }}
            </x-responsive-nav-link>
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-red-800">
            @auth
                ...
                <div class="mt-3 space-y-1">
                    <x-responsive-nav-link :href="route('profile.edit')" class="text-gray-200 hover:bg-red-800 block px-3 py-2 text-base font-medium">
                        {{ __('Profile') }}
                    </x-responsive-nav-link>

                    <x-responsive-nav-link :href="route('settings.edit')" class="text-gray-200 hover:bg-red-800 block px-3 py-2 text-base font-medium">
                        {{ __('Settings') }}
                    </x-responsive-nav-link>

                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <x-responsive-nav-link :href="route('logout')" class="text-gray-200 hover:bg-red-800 block px-3 py-2 text-base font-medium" onclick="event.preventDefault(); this.closest('form').submit();">
                            {{ __('Log Out') }}
                        </x-responsive-nav-link>
                    </form>
                </div>
