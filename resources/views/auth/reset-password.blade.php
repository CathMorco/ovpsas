...
<head>
    ...
    <title>Reset Password - Student Affairs</title>
...
<body class="bg-gray-100 font-sans antialiased flex flex-col min-h-screen">
    <main class="flex-grow flex items-center justify-center relative"
        ...
        <div class="bg-white p-10 md:p-14 rounded-[3rem] shadow-2xl w-full max-w-lg mx-4">
            <h2 class="text-xl font-semibold text-gray-800 mb-6 text-center">Set New Password</h2>
            ...
            <form method="POST" action="{{ route('password.store') }}" class="flex flex-col gap-6">
                ...
                <div class="space-y-4">
                    <input type="email" name="email" value="{{ old('email', $request->email) }}" required readonly
                        class="w-full bg-[#E8EDF2] border-none rounded-xl p-4 text-gray-400 cursor-not-allowed italic">
