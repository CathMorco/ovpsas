...
<head>
    ...
    <title>Log In - OVPSAS Portal</title>
...
<body class="bg-gray-100 font-sans antialiased flex flex-col min-h-screen">
    ...
    <main class="flex-grow flex items-center justify-center relative"
        ...
        <div class="bg-white p-10 md:p-14 rounded-[3rem] shadow-2xl w-full max-w-lg mx-4">
            @if (session('status'))
                <div class="mb-4 font-medium text-sm text-green-600 text-center border border-green-200 bg-green-50 p-3 rounded-xl">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-6">
                ...
                <div class="text-center space-y-3 pt-2">
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm text-gray-800 hover:underline block italic font-bold">Lost password?</a>
                    @endif
                    <p class="text-sm text-gray-800 uppercase font-black tracking-tighter">
                        Don't have an account?
                        <a href="{{ route('register') }}" class="text-[#800000] hover:underline">Create new account</a>
                    </p>
                </div>
            </form>
        </div>
